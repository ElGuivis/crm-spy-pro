...
const log = createLogger('MelhorEnvioCronStatus');
...
// Parse cron schedule "0 * * * *" = every hour at minute 0
...
export const MelhorEnvioCronStatus = () => {
  ...
  const fetchCronStatus = async () => {
    try {
      // Fetch cron job info for Melhor Envio
      const { data: jobData } = await supabase.rpc('get_me_cron_job_status');
      ...
      // Fetch last run
      const { data: runData } = await supabase.rpc('get_me_cron_last_run');
      
      if (runData && Array.isArray(runData) && runData.length > 0) {
        setLastRun(runData[0]);
      }
    } catch (error) {
      log.error("Error fetching ME cron status:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!cronJob) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm">
        ...
        <span className="text-muted-foreground">Auto-sync não configurado</span>
      </div>
    );
  }
        ...
        <TooltipContent side="bottom" className="max-w-xs">
          <div className="space-y-1 text-xs">
            ...
            <p className="text-muted-foreground mt-2">
              Sincroniza envios e rastreamentos automaticamente a cada hora.
            </p>
          </div>
        </TooltipContent>
      </Tooltip>
