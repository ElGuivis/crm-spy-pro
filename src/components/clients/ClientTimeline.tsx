...
interface ClientTimelineProps {
  client: Tables<"li_customers">;
  integrationId: string;
}

export function ClientTimeline({ client, integrationId }: ClientTimelineProps) {
  const { data: orders = [] } = useQuery({
    queryKey: ["client-orders-timeline", client.id],
    queryFn: async () => {
      const { data } = await supabase
        .from("li_orders")
        .select("id, order_number, status_name, created_at_remote, totals_json")
  ...
  const { data: loyaltyRows = [] } = useQuery({
    queryKey: ["client-loyalty-timeline", integrationId, customerKey],
    queryFn: async () => {
  ...
  const events: TEvent[] = [
    ...orders.map((o) => ({
      id: `order-${o.id}`,
      date: new Date(o.created_at_remote || ""),
      type: "order" as const,
          ...
          <div className="flex-1 min-w-0 pt-0.5">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="text-right shrink-0">
                {event.amount && (
                  <p className={`text-xs font-semibold ${amountColor(event.type)}`}>{event.amount}</p>
                )}
